...
userHasSurfSkillsController.get('/:id_user', (async (req: Request, res: Response) => {
    const { id_user } = req.params
    const userSurfSkills = await UserHasSurfSkills.getSurfSkillsByUser(parseInt(id_user, 10))
    console.log(userSurfSkills)
    res.status(200).json(userSurfSkills)
}) as RequestHandler)


userHasSurfSkillsController.post('/:id_user/:id_surf_skill', (async (req: Request, res: Response) => {
    const { id_user, id_surf_skill } = req.params
    try {
        const created = await UserHasSurfSkills.create(parseInt(id_user, 10), parseInt(id_surf_skill))
...
userHasSurfSkillsController.delete('/:id_user/:id_surf_skill', (async (req: Request, res: Response) => {
    const { id_user, id_surf_skill } = req.params
    try {
