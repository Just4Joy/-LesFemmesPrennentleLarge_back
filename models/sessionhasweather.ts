...
const getWeatherBySession = (id_session: ISessionhasweather['id_session']) => {
  return connection
    .promise()
    .query<ISessionhasweather[]>(
      'SELECT * FROM sessions_has_weather WHERE id_session = ?',
      [id_session]
    )
    .then(([result]) => result);
};
...
const create = (
  id_session: ISessionhasweather['id_session'],
  id_weather: ISessionhasweather['id_weather']
) => {
  return connection
    .promise()
    .query<ISessionhasweather[]>(
...
const destroy = (
  id_user: ISessionhasweather['id_user'],
  id_weather: ISessionhasweather['id_surf_skill']
) => {
  return connection
    .promise()
    .query<ISessionhasweather[]>(
