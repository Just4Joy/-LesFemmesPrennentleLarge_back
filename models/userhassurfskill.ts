...
const getSurfSkillsByUser = (id_user: IUserhassurfskills['id_user']) => {
  return connection
    .promise()
    .query<IUserhassurfskills[]>(
      'SELECT * FROM users_has_surf_skills WHERE id_user = ?',
      [id_user]
    )
    .then(([result]) => result);
};
...
const create = (
  id_user: IUserhassurfskills['id_user'],
  id_surf_skill: IUserhassurfskills['id_surf_skill']
) => {
  return connection
    .promise()
    .query<IUserhassurfskills[]>(
